...
const userInactivityTracker = async () => {
  try {
    const getLastAppOpenedTimestampResponse = await Hasura(
      getLastAppOpenedTimestamp
    );
    const lastAppOpenedLogs =
      getLastAppOpenedTimestampResponse.result.data.user_actions;
    ...
    console.error(
      "Error checking app opened and updating consecutive days:",
      error
    );
  }
};
